...
namespace MixerLib
{
	internal class ConstellationEventParser : IEventParser
	{
		...
		{
			_fireEvent(nameof(MixerClientInternal.Followed),
		...
		{
			_fireEvent(nameof(MixerClientInternal.Subscribed), new SubscribedEventArgs { ChannelId = channelId, UserName = payload.User.Username });
		}

		void HandleResubscribed(uint channelId, WS.ResubSharedPayload payload)
		{
			_fireEvent(nameof(MixerClientInternal.Resubscribed),
